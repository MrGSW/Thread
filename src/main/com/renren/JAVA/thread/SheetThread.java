	...
	public void run() {
		try {
			...
			if (rows > 1) {
				...
				CountDownLatch cellLath = new CountDownLatch(READ_SHEET_CELL_THREAD_NUMBER);
				int stepLength = getLength(rows, READ_SHEET_CELL_THREAD_NUMBER);
				for (int index = 0; index < rows; index += stepLength) {
					executorService.submit(new ReadSheetThread(cellLath, this.sheet, index, getEndIndex(index, stepLength, rows)));
				}
				cellLath.await();
